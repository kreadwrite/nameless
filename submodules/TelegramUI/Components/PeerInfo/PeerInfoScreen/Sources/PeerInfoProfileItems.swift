...
private func isNamelessOfficialUser(_ user: TelegramUser) -> Bool {
    if user.id.id._internalGetInt64Value() == namelessOfficialUserId {
        return true
    }
            ...
            if let reactionSourceMessageId = reactionSourceMessageId {
                ...
                if isNamelessOfficialUser(user) {
                    items[currentPeerInfoSection]!.append(PeerInfoScreenCommentItem(
                        ...
                        icon: .nameless,
                        text: "This account is verified as official by representatives of nameless.",
