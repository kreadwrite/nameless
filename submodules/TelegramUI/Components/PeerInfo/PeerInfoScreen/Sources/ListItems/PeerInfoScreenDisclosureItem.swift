...
final class PeerInfoScreenDisclosureItem: PeerInfoScreenItem {
    ...
    init(id: AnyHashable, label: Label = .none, additionalBadgeLabel: String? = nil, additionalBadgeIcon: UIImage? = nil, text: String, icon: UIImage? = nil, iconSignal: Signal<UIImage?, NoError>? = nil, hasArrow: Bool = true, action: (() -> Void)?, longTapAction: (() -> Void)? = nil) {
        self.id = id
        self.label = label
    ...
    override func update(context: AccountContext, width: CGFloat, safeInsets: UIEdgeInsets, presentationData: PresentationData, item: PeerInfoScreenItem, topItem: PeerInfoScreenItem?, bottomItem: PeerInfoScreenItem?, hasCorners: Bool, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        if item.longTapAction != nil {
            if self.longTapRecognizer == nil {
                ...
                self.activateArea.view.addGestureRecognizer(recognizer)
            }
        } else if let recognizer = self.longTapRecognizer {
            self.activateArea.view.removeGestureRecognizer(recognizer)
            self.longTapRecognizer = nil
        }
