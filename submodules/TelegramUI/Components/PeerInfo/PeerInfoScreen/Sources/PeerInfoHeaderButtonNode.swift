...
final class PeerInfoHeaderButtonNode: HighlightableButtonNode {
    ...
    func update(size: CGSize, text: String, icon: PeerInfoHeaderButtonIcon, isActive: Bool, presentationData: PresentationData, backgroundColor: UIColor, foregroundColor: UIColor, fraction: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let roundDiameter = min(size.width, size.height)
        let backgroundY: CGFloat = useRoundButtons ? floor((size.height - roundDiameter) / 2.0) : size.height * (1.0 - fraction)
        let backgroundFrame: CGRect
        if useRoundButtons {
            backgroundFrame = CGRect(origin: CGPoint(x: floor((size.width - roundDiameter) / 2.0), y: backgroundY), size: CGSize(width: roundDiameter, height: roundDiameter))
        } else {
            backgroundFrame = CGRect(origin: CGPoint(x: 0.0, y: backgroundY), size: CGSize(width: size.width, height: max(0.0, size.height - backgroundY)))
        }
        ...
        transition.updateCornerRadius(layer: self.backgroundView.layer, cornerRadius: useRoundButtons ? backgroundFrame.height * 0.5 : min(16.0, backgroundFrame.height * 0.5))
        ...
        let iconY: CGFloat = useRoundButtons ? floor((size.height - iconSize.height) / 2.0) : 1.0
        ...
        transition.updateAlpha(node: self.textNode, alpha: useRoundButtons ? 0.0 : 1.0)
